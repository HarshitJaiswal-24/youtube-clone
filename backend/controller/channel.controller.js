...
// New: update a video's details (title, description, optional thumbnail)
export const updateVideo = async (req, res) => {
  try {
    const { channelId, videoId, title, description } = req.body;

    if (!channelId || !videoId) {
      return res.status(400).json({ msg: "channelId and videoId are required" });
    }
    ...
    const targetVideo = Channel.videos.find(
    ...
    if (typeof title === "string" && title.trim() !== "") {
      targetVideo.title = title.trim();
    }
    if (typeof description === "string") {
      targetVideo.description = description;
    }

    const thumbnailFile = req.files?.["thumbnail"]?.[0];
    if (thumbnailFile) {
      targetVideo.thumbnail = `/uploads/${thumbnailFile.filename}`;
    }

    await Channel.save();
    return res.status(200).json({ msg: "Video updated successfully", video: targetVideo });
  } catch (err) {
