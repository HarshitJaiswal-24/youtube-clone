...
export const AuthProvider = (props) => {
  ...
  async function fetchUser() {
    try {
      ...
      const res = await axios.get("http://localhost:3000/user", { withCredentials: true });// Include cookies for authentication
 ...
 // Fetch user when the provider first mounts
  ...
  async function signout() {
    try {
      await axios.post("http://localhost:3000/user/signout", {}, { withCredentials: true });
      setUser(null);
    } catch (err) {
