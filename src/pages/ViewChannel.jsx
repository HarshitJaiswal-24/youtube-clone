...
export default function ViewChannel() {
    ...
    const fetchChannel = async () => {
        try {
            const res = await axios.get(`http://localhost:3000/channels/${id}`, { withCredentials: true });
            setChannel(res.data);
        } catch (err) {
    ...
    const fetchVideos = async () => {
        try {
            const res = await axios.get(`http://localhost:3000/videos/channel/${id}`);
            setVideos(res.data);
        } catch (err) {
    ...
    const handleEditSubmit = async (videoId) => {
        try {
            await axios.put(`http://localhost:3000/${videoId}`, {
                title: editingTitle,
    ...
    const handleDeleteVideo = async (videoId) => {
        try {
            await axios.delete(`http://localhost:3000/${videoId}`, { withCredentials: true });
            fetchVideos();
        } catch (err) {
            console.error(err);
        }
